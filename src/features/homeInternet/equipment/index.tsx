    ...
    const fetchOptionalProducts = async () => {
      ...
      try {
        ...
        if (res.status && Array.isArray(res.data) && res.data.length > 0) {
          const installation: OptionalProduct | undefined = res.data.find((p: OptionalProduct) =>
            p.name.toLowerCase().includes("installation fee"),
          );
          const delivery: OptionalProduct | undefined = res.data.find((p: OptionalProduct) =>
            p.name.toLowerCase().includes("delivery fee"),
          );
          ...
          if (installation) {
            ...
            if (existingFees) {
              ...
              const hasDelivery =
                delivery && parsed.some((f: any) => f.variantId === delivery.product_variant_id[0]);
