...
export default function Plan() {
  ...
  useEffect(() => {
    if (search.get("location") && search.get("businesstype")) {
      const selectedData = plans.find((item) => item.id === selected);
      if (selectedData) {
        router.push(
          `/business-internet/plan?businesstype=${search.get("businesstype")}&location=${search.get("location")}&label=${selectedData.name}&value=${selectedData.price}&type=price`,
        );
      }
    }
  }, []);
          ...
          <div className="w-full lg:max-w-3xl bg-white rounded-xl p-4 xl:p-8 shadow-sm">
            ...
            <div className="text-sm text-[#6b7280] mb-2">
              <div className="flex gap-1 items-center">
                ...
                <button className="ml-2 text-[#2563eb] underline">
                  <Link
                    href={`/business-internet/location?businesstype=${search.get("businesstype")}`}
                  >
                    Change
                  </Link>
                </button>
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          setSelected(plan.id);
                          router.push(
                            `/business-internet/plan?businesstype=${search.get("businesstype")}&location=${search.get("location")}&label=${plan.name}&value=${plan.price}&type=price`,
                          );
                        }}
            ...
            <div className="flex flex-col lg:flex-row gap-y-4 justify-between mt-6">
              <button
                ...
                onClick={() => {
                  router.push(
                    `/business-internet/location?businesstype=${search.get("businesstype")}`,
                  );
                }}
              >
                Back
              ...
              <button
                ...
                onClick={() => {
                  if (search.get("location")) {
                    ...
                    if (selectedData) {
                      router.push(
                        `/business-internet/extras?businesstype=${search.get("businesstype")}&location=${search.get("location")}&label=${selectedData.name}&value=${selectedData.price}&type=price`,
                      );
                    }
                  }
                }}
